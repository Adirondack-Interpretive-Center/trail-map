...
function togglePage(page) {
  //console.log(page)
  if (page=='summer') {
    map.addLayer(layers.basemaps["Summer Trail Map"])
    map.removeLayer(layers.basemaps["Winter Trail Map"])
  }
  else if (page=='winter') {
    map.addLayer(layers.basemaps["Winter Trail Map"])
    map.removeLayer(layers.basemaps["Summer Trail Map"])
  }
  else {
    $("#trees").show();
    $("#invasives").hide();
...
const layers = {
  basemaps: {
    ...
    "Winter Trail Map": L.tileLayer.mbTiles("data/winter_map.mbtiles", {
      autoScale: true,
      fitBounds: true,
      updateWhenIdle: false
    }).on("databaseloaded", (e) => {
      map.setMaxBounds(L.latLngBounds(layers.basemaps["Winter Trail Map"].options.bounds).pad(0));
      controls.locateCtrl.start();
      ZoomToExtent();
    }).addTo(map),
    "Summer Trail Map": L.tileLayer.mbTiles("data/summer_map.mbtiles", {
      autoScale: true,
      fitBounds: false,
      updateWhenIdle: false
    }).on("databaseloaded", (e) => {
      //layers.basemaps["Summer Trail Map"].bringToFront();
      //map.removeLayer(layers.basemaps["Winter Trail Map"])
...
function ZoomToExtent() {
  map.fitBounds([
    [43.970036450793934, -74.19204725311559],
    [43.98020855456357, -74.18359332790638]
]);
}
...
initSqlJs({
  ...
  hideLoader();
  //loadData();
  //layers.basemaps["Summer Trail Map"].addTo(map);
